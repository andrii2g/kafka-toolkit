...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help) echo "Usage: kafka-sample-keys.sh --topic TOPIC [--count NUMBER] [--from-beginning]"; exit 0 ;;
...
consumer_args=(--bootstrap-server "$BOOTSTRAP" "${config_args[@]}" --topic "$TOPIC" --max-messages "$COUNT" --property print.key=true --property print.value=false --property key.separator=$'\t')
[[ "$FROM_BEGINNING" -eq 1 ]] && consumer_args+=(--from-beginning)

timeout "$TIMEOUT" "$KAFKA_CONSUMER" "${consumer_args[@]}" 2>/dev/null | awk -F '\t' '{ print $1 }'
